...
function buildChapter(opts: {
  ...
  const { characterName, locationSlug, titleKo, genre, worldview, keyword } = opts
...
export const setupCommand = new Command("setup")
  ...
  .action(async (dir: string) => {
    ...
    // ─── Step A: World questions ───────────────────────────────────────────────
    console.log("")
    console.log("[Opencanon] 함께쓰는 세계관 창작을 위해 아래와 같은 질문이 있네요")
    console.log("양식없이 편하게 답변해주세요:\n")

    const characterName = await ask("1. 주인공의 이름은 무엇인가요? ")
    ...
    const genre = await ask("2. 어떤 장르인가요? (예: SF, 판타지, 현대물, 로맨스) ")
    const worldview = await ask("3. 대략적인 세계관 아이디어가 있다면 편하게 적어주세요\n   (시대 배경, 갈등구조, 사랑이야기, 인간승리 등): ")

    // ─── Step B: Keyword ──────────────────────────────────────────────────────
    console.log("")
    console.log("이제 세계관 창조가 준비되었습니다.")
    const keyword = await ask("당신의 세계에 심어줄 하나의 키워드는 무엇인가요?\n(상처, 고통, 아름다움, 즐거움, 연결, ...): ")

    rl.close()

    // ─── Step C: Generate everything ──────────────────────────────────────────
    console.log("")
    console.log("세계를 생성하고 있습니다...")
    console.log("")
    ...
    const worldTitle = characterName ? `${characterName}의 세계` : "새로운 세계"
    const repoSlug = toSlug(worldTitle) || "my-world"
    ...
    const titleKo = `${characterName}의 시작`
    const titleEn = `The Beginning of ${characterName}`
    ...
    execSync(`git commit -m "[setup] ${worldTitle} — ${genre} / ${keyword}"`, { cwd: root, stdio: "pipe" })
    ...
    // Summary
    console.log("────────────────────────────────────────")
    console.log(`세계 이름:  ${worldTitle}`)
    console.log(`주인공:    ${characterName} (${characterSlug})`)
    console.log(`장르:      ${genre}`)
    console.log(`키워드:    ${keyword}`)
    console.log(`첫 장면:   stories/${ep01Slug}/chapter-01.md`)
    console.log("")
    console.log("다음 단계:")
